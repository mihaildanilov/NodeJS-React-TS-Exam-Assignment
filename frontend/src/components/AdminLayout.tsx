...
const AdminLayout = () => {
	...
	useEffect(() => {
		...
		handleResize(); // initialize the state based on the current width
		window.addEventListener('resize', handleResize); // add NavLink resize event listener

		return () => {
			window.removeEventListener('resize', handleResize); // remove the resize event listener on unmount
		};
	}, []);
	return (
		<div>
